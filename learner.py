...
from mpnn import MPNN, MultiHeadAttention
...
def setup_master(args, env=None, return_env=False):
    ...
    if args.env_name == 'simple_spread':
        ...
        num_entities = 2
    elif args.env_name == 'point_to_point':
        num_entities = 1
    elif args.env_name == 'new_formation':
        num_entities = 1
    else:
    ...
    if entity_mp:
        ...
        pol_obs_dim = env.observation_space[i].shape[0]    # len(senario.observation())
    ...
    for i, agent in enumerate(env.world.policy_agents):
        ...
        if hasattr(agent, 'adversary') and agent.adversary:
            if policy1 is None:
                ...
                # model = policy1   # 自己加的
                # with SummaryWriter(comment="MPNN") as w:
                #     w.add_graph(model=model, input_to_model=torch.rand(1, 1, 28, 28))
            ...
            if policy2 is None:
                policy2 = MPNN(input_size=pol_obs_dim, num_agents=num_friendly, num_entities=num_entities, action_space=action_space,
                               pos_index=pos_index, mask_dist=args.mask_dist, entity_mp=entity_mp).to(args.device)
                # model = policy2.messages
                # with SummaryWriter(comment="MPNN") as w:
                #     w.add_graph(model=model, input_to_model=torch.rand(32, 5, 2))

            team2.append(Neo(args, policy2, (obs_dim,), action_space))
    master = Learner(args, [team1, team2], [policy1, policy2], env)
    # # ***************
    # all_obs = torch.cat([agent.rollouts.obs[1] for agent in team2])
    # model = policy2
    # with SummaryWriter(comment="MPNN") as w:
    #     w.add_graph(model=model, input_to_model=torch.rand(all_obs.size()))
    # # *************
...
class Learner(object):
    ...
    def __init__(self, args, teams_list, policies_list, env):
        ...
        self.all_agents = [agent for team in teams_list for agent in team]   # team2
        self.policies_list = [x for x in policies_list if x is not None]     # policy2
    ...
    def act(self, step):    #在此step是0/1/2..../127
        actions_list = []
        for team, policy in zip(self.teams_list, self.policies_list):
            ...
            # split all outputs
            n = len(team)
            all_value, all_action, all_action_log_prob, all_states = [torch.chunk(x, n) for x in props]    # 网络输出的所有智能体的结果!!!!!!
            for i in range(n):
    ...
    def update(self):
        ...
        for i, trainer in enumerate(self.trainers_list):
            rollouts_list = [agent.rollouts for agent in self.teams_list[i]]    # 智能体数目个rollouts
            vals = trainer.update(rollouts_list)    # 跳到jointppo.update
    ...
    def update_rollout(self, obs, reward, masks):
        ...
        for i, agent in enumerate(self.all_agents):
            agent_obs = obs_t[:, i, :]    # 线程数 * 第i个智能体 * obs大小
