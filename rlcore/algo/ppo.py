...
class JointPPO():
    ...
    def update(self, rollouts_list):
        ...
        for e in range(self.ppo_epoch):   # default = 4
            if self.actor_critic.is_recurrent:    # false
            ...
            for sample in data_generator:
                ...
                masks_batch, old_action_log_probs_batch, adv_targ = sample   # 得到的是n个智能体的
                # Reshape to do in a single forward pass for all steps
                values, action_log_probs, dist_entropy, _ = self.actor_critic.evaluate_actions(obs_batch,
                                 recurrent_hidden_states_batch, masks_batch, actions_batch)

                ratio = torch.exp(action_log_probs - old_action_log_probs_batch)   # 重要性采样处理动作分布？？？？？
                surr1 = ratio * adv_targ    # 《深入浅出强化学习》150页公式（8.9）
                surr2 = torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param) * adv_targ
                action_loss = -torch.min(surr1, surr2).mean()
