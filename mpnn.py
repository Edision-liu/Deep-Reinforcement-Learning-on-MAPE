...
class MPNN(nn.Module):
    def __init__(self, action_space, num_agents, num_entities, input_size=16, hidden_dim=128, embed_dim=None,
        ...
        self.input_size = input_size   # len(senario.observation())~~~5
    ...
    def act(self, inp, state, mask=None, deterministic=False):
        x = self._fwd(inp)
        value = self._value(x)
        dist = self.dist(self._policy(x))     # policy的距离????
        # print(dist)
        if deterministic:
            action = dist.mode()
        else:
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if val_dim is None:
            ...
            val_dim = embed_dim // n_heads   # 128//1
        if key_dim is None:
            key_dim = val_dim     # 128
        ...
        self.norm_factor = 1 / math.sqrt(key_dim)  # See Attention is all you need# 对 dot-product attention 进行缩放（除以 d k \sqrt{d_k}
    ...
    def forward(self, q, h=None, mask=None, return_attn=False):
        ...
        hflat = h.contiguous().view(-1, input_dim)    # *行，input_dim列
...
#
#     env = utils.make_parallel_envs(args)    #创建环境
#     pol_obs_dim = env.observation_space[20].shape[0]
#     model = MPNN(action_space=pol_obs_dim, num_agents=10, num_entities=1)
