...
import numpy as np
from mape.multiagent.multi_discrete import MultiDiscrete
...
class MultiAgentEnv(gym.Env):
    ...
    def step_try(self, action_n):
        obs_n = []
        self.agents = self.world.policy_agents
        # set action for each agent
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.action_space[i])
    def step(self, action_n):    # action_n 为n个智能体的行为构成的列表
        obs_n = []
        reward_n = []
        done_n = []
        info_n = {'n': []}
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                # print(self.discrete_action_input)
                agent.action.u = np.zeros(self.world.dim_p)
                # process discrete action
                push_force = 0.5
                if np.size(self.world.dists)!=0:
                    distss = self.world.dists[agent.iden]
                    # print(distss)
                    push_force = 0.3 + (distss-0.05) / 10      ##需要考虑根据formation圆形大小进行调节！！！！！！！！！
                    if push_force < 0.3:
                        push_force = 0.3
                    # print(push_force)

                if action[0] == 1: agent.action.u[0] = -push_force
                if action[0] == 2: agent.action.u[0] = +push_force
                if action[0] == 3: agent.action.u[1] = -push_force
                if action[0] == 4: agent.action.u[1] = +push_force
                # if action[0] == 5: agent.action.u[0] = -0.2
                # if action[0] == 10: agent.action.u[0] = +0.2
                # if action[0] == 15: agent.action.u[1] = -0.2
                # if action[0] == 20: agent.action.u[1] = +0.2
                #  !!!!!!!!!!!!!!!!!!加入静止动作
                if action[0] == 0: agent.action.u[0] = 0
            else:
    ...
    def render(self, mode='human', attn=None):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from mape.multiagent import rendering
